...
async def change_password_endpoint(
    ...
    try:
        ...
        await auth_service.change_password_service(
            session=session, 
            user_uuid=user_id, 
        ...
        status_code = 400
        if "Mật khẩu cũ không chính xác" in str(e):
            status_code = 400
        raise HTTPException(status_code=status_code, detail=f"Lỗi đổi mật khẩu: {e}")